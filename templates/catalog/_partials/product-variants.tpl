    ...
    <div class="clearfix product-variants-item">
      ...
      {elseif $group.group_type == 'color'}
        <ul id="group_{$id_attribute_group}">
          {foreach from=$group.attributes key=id_attribute item=group_attribute}
          <li class="float-xs-left input-container color-select {if $group_attribute.selected}selected{/if}">
              <label aria-label="{$group_attribute.name}">
                ...
                <span
                  ...
                  {/if}
                ><span class="attribute-name">{$group_attribute.name}</span></span>
              </label>
            </li>
          {/foreach}
        </ul>
      {elseif $group.group_type == 'radio'}
        <ul id="group_{$id_attribute_group}">
          {foreach from=$group.attributes key=id_attribute item=group_attribute}
            <li class="input-container float-xs-left {if $group_attribute.selected}selected{/if}">
              <label>
